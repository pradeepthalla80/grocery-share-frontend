...
// FIXED: Changed status filter from ['available', 'pending', 'picked_up'] to 'available' only
// UPDATED: Added averageRating and ratingCount to user populate
const getItemsByLocation = async (req, res) => {
  try {
    ...
    const query = {
      ...
      // FIXED: Only show 'available' items in default search
      // Items in 'awaiting_pickup' or 'completed' status should not appear
      status: 'available'
    };
    ...
    const items = await Item.find(query).populate('user', 'name email storeName averageRating ratingCount');
...
// Schedule pickup (protected route)
const schedulePickup = async (req, res) => {
  try {
    const { id } = req.params;
    const { buyerId, pickupDate, pickupTime, deliveryOption, deliveryAddress } = req.body;
    ...
    if (!deliveryOption || !['pickup', 'delivery'].includes(deliveryOption)) {
      return res.status(400).json({ error: 'Valid delivery option is required (pickup or delivery)' });
    }
    ...
    res.json({
      ...
      item: {
        ...
        pickupScheduled: item.pickupScheduled,
    ...
    res.status(500).json({ 
      error: 'Error scheduling pickup',
      details: error.message 
    });
  }
};

// Update pickup status (protected route)
const updatePickupStatus = async (req, res) => {
  try {
    ...
    // Check if user is owner or buyer (convert both to strings for comparison)
    const isOwner = item.user.toString() === req.user.userId.toString();
    const isBuyer = item.buyerId && item.buyerId.toString() === req.user.userId.toString();
