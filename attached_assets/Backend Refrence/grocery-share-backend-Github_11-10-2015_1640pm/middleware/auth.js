...
const auth = async (req, res, next) => {
  try {
    // Try to get token from cookie first (works with tracking prevention)
    ...
    // Attach user to request (with userId for backwards compatibility)
    req.user = {
      _id: user._id,
      userId: user._id,
      email: user.email,
...
// Export for use as both auth and protect
module.exports = auth;
