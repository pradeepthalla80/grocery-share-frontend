...
// routes/admin.js
// 
// IMPORTANT: Check your middleware/auth.js file
// If the admin middleware is exported as something else
// (like 'isAdmin' or 'adminMiddleware'), update line 8
// ============================================

const express = require('express');
const router = express.Router();
// CHECK YOUR BACKEND: Make sure 'admin' matches your middleware export name
// It might be 'isAdmin' or 'adminMiddleware' instead
...
router.delete('/users/:userId', protect, admin, async (req, res) => {
  try {
    ...
    // Delete user's items, requests, messages, ratings, etc.
    await Promise.all([
...
router.put('/users/:userId/store-status', protect, admin, async (req, res) => {
  try {
    ...
    // If disabling store mode, also disable storeMode
    if (!isStoreOwner) {
