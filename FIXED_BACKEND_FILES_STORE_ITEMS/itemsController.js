...
const createItem = async (req, res) => {
  try {
    ...
    const isStoreItemFlag = isStoreItem === true || isStoreItem === 'true';
    
    if (!name || !expiryDate || !location) {
      return res.status(400).json({ error: 'Please provide name, expiryDate, and location' });
    }
    
    if (!isItemFree && (!price || parseFloat(price) <= 0)) {
      return res.status(400).json({ error: 'Please provide a valid price or mark the item as free' });
    }
    ...
    if (locationData.lat === undefined || locationData.lat === null || 
      ...
      return res.status(400).json({ error: 'Location must include lat and lng coordinates' });
    }
    ...
    if (validityPeriod && validityPeriod !== 'never') {
      const now = new Date();
      if (validityPeriod === '2h') expiresAt = new Date(now.getTime() + 2 * 60 * 60 * 1000);
      else if (validityPeriod === '6h') expiresAt = new Date(now.getTime() + 6 * 60 * 60 * 1000);
      else if (validityPeriod === '12h') expiresAt = new Date(now.getTime() + 12 * 60 * 60 * 1000);
      else if (validityPeriod === '24h') expiresAt = new Date(now.getTime() + 24 * 60 * 60 * 1000);
      else {
    ...
    if (isStoreItemFlag && quantity) {
      parsedQuantity = parseInt(quantity);
      if (isNaN(parsedQuantity) || parsedQuantity < 0) {
        return res.status(400).json({ error: 'Quantity must be a positive number' });
      }
    }
    const finalStockStatus = isStoreItemFlag ? (stockStatus || 'in_stock') : undefined;
    ...
    if (isStoreItemFlag) {
      itemData.isStoreItem = true;
      itemData.quantity = parsedQuantity;
      itemData.stockStatus = finalStockStatus;
    }
    ...
    res.status(500).json({ error: 'Error creating item', details: error.message });
  }
};
