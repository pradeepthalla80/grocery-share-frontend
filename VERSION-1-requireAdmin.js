// ============================================
// VERSION 1: If your backend uses 'requireAdmin'
...
router.get('/items', protect, requireAdmin, async (req, res) => {
  try {
...
// Update user role
router.put('/users/:userId/role', protect, requireAdmin, async (req, res) => {
  try {
    const { role } = req.body;
...
router.put('/users/:userId/store-status', protect, requireAdmin, async (req, res) => {
  try {
