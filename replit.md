...
- **Image Management**: Multi-image uploads (up to 5 per item) with Cloudinary integration. **Mobile Fix (Nov 16, 2025)**: Image preview component uses padding-bottom: 100% technique with absolute positioning for perfect square aspect ratios on all browsers, including mobile Safari, eliminating black box display issues.
...
- **Communication**: One-on-one messaging via modal popup, conversation threads with visual unread badges (red color highlighting), real-time polling (3-second intervals), URL-driven chat entry, pre-filled messages, and an address reveal mechanism. Unread messages display with red backgrounds, borders, and "New" badges for easy identification.
