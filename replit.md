...
- **Image Management**: Multi-image uploads (up to 5 per item) with Cloudinary integration. **Mobile Fix (Nov 16, 2025)**: Image preview component uses CSS `aspectRatio: '1/1'` property with `overflow: hidden` and `display: block` for perfect square aspect ratios on all browsers, including mobile Safari, eliminating black box display issues.
