...
- **BottomNav Portal:** BottomNav renders via React `createPortal` to `document.body`. Uses `<a href>` tags (not buttons) with `onClick` calling `window.location.href` for guaranteed iOS Chrome reliability. If React's onClick doesn't fire, the browser's native `<a>` href navigation still works. z-index 99999 + `pointerEvents: auto`.
- **Hamburger Menu Portal:** Navbar mobile drawer also renders via `createPortal` to `document.body` with z-index 100001 (backdrop 100000). All menu items use `<a href>` tags with `onClick → window.location.href` for iOS reliability. Drawer auto-closes on route change via `useEffect` on `location.pathname`.
- **iOS Touch CSS:** All `button` elements have `touch-action: manipulation` (removes 300ms tap delay on iOS) and `-webkit-tap-highlight-color: transparent`.
- **Leaflet Touch Fix:** `LocationMap` has `dragging={false}`, `touchZoom={false}`, `doubleClickZoom={false}`, `zoomControl={false}` — prevents Leaflet from registering global document-level touch listeners that intercepted ALL touch events on the AddItem page (causing BottomNav and Stripe button to be unresponsive).
- **Stripe Button Fix:** AddItem page Stripe setup button is an `<a href="/profile">` tag (not a form button) for reliable iOS tap handling.
...
- **Stripe Link UX:** AddItem page shows a full-width green `<a href="/profile">` for Stripe setup where both store-owners and regular users can complete Stripe onboarding. Using `<a>` instead of `<button>` ensures reliable tap handling on iOS Chrome inside a form.
