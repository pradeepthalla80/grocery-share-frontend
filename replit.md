...
- **Hamburger Menu Portal:** Navbar mobile drawer also renders via `createPortal` to `document.body` with z-index 100001 (backdrop 100000), above the BottomNav. All menu items use `<button type="button">` with `navigate()` instead of `<Link>` for iOS Safari touch reliability. Drawer auto-closes on route change via `useEffect` on `location.pathname`.
