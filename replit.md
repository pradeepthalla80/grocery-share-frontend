...
- **Backend Structure**: Backend repository uses root-level `controllers/`, `routes/`, `models/`, and `services/` folders. Main entry point is `index.js` at root level (NOT in src/). Files: `controllers/analyticsController.js`, `controllers/storeController.js`, `controllers/pickupRequestController.js`, `routes/analytics.js`, `routes/store.js`, `routes/pickupRequests.js`, `models/StoreAgreement.js`, `models/PickupRequest.js`, `models/Item.js` (updated with new statuses).
