...
- **Backend Structure**: Backend repository uses root-level `controllers/`, `routes/`, `models/`, and `services/` folders. Main entry point is `index.js` at root level (NOT in src/). Files: `controllers/analyticsController.js`, `controllers/storeController.js`, `routes/analytics.js`, `routes/store.js`, `models/StoreAgreement.js`.
- **Deployment Status**: Backend deployed on Render with auto-deploy from GitHub (commit `7c17f43`, deployed Nov 10, 2025 @ 4:40 PM). All Store Owner Mode endpoints live and functional. Frontend deployed on Vercel.
