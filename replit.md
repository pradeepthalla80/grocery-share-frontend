...
- **Image Management**: Multi-image uploads (up to 5 per item) with Cloudinary integration. **Mobile Fix (Nov 16, 2025)**: Image preview component uses padding-bottom: 100% intrinsic ratio technique with absolute positioning to fix iOS Safari WebKit bug that caused black box display issues. This classic wrapper approach ensures perfect square aspect ratios on all browsers, including mobile Safari.
